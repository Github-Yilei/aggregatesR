#' @title plotting_funcs
#'
#' @description This R script is provided with treemix. Further details are available on the paper:
...
#' plot_tree
#'
#' @description Plotted is the structure of the graph inferred by TreeMix for sub-populations, allowing
#' different migration events. Migration arrows are colored according to their weight. Horizontal branch lengths are proportional to the amount of genetic
#' drift that has occurred on the branch.
#' @param stem the given folder of the output of Treemix with the suffixes “.llik,” “.cov.gz,” and “.modelcov.gz” generated by default using Treemix.
#' @examples
#' file_path <- system.file("treemix", package = "aggregatesR" )
#' setwd(file_path)
#' par(mfrow=c(3,3))
#' for(edge in 0:10){
#'    plot_tree(paste0("treemix.3.", edge))
...
#' plot_modelcov
#'
#' @description plot the modelcov
#' @param stem the given folder of the output of Treemix with the suffixes “.llik,” “.cov.gz,” and “.modelcov.gz” generated by default using Treemix.
#' @param pop_order A file that contains the order of pops that you wanted to be plotted.
#' @examples
#' file_path <- system.file("treemix", package = "aggregatesR" )
#' setwd(file_path)
#' par(mfrow=c(3,2))
#' for(edge in 0:5){
#'     plot_modelcov(stem=paste0("treemix.3.", edge),
...
#' par(mfrow=c(3,2))
#' for(edge in 0:5){
#'     plot_cov(stem=paste0("treemix.3.", edge),
#'     pop_order="pop_order.txt")
#'     }
#' @export
#'
...
#' plot_resid
#'
#' @description Positive residuals indicate pairs of populations where the model
#' underestimates the observed covariance, and thus populations
#' where the fit might be improved by adding additional edges.
#' Negative residuals indicate pairs of populations where the model
#' overestimates the observed covariance; these are a necessary
#' outcome of having positive residuals, but can also sometimes be
#' interpreted as populations that are forced too close together due to
#' unmodeled migration elsewhere in the graph.
#'
#' @param stem the given folder of the output of Treemix with the suffixes “.llik,” “.cov.gz,” and “.modelcov.gz” generated by default using Treemix.
#' @param pop_order A file that contains the order of pops that you wanted to be plotted.
#'
...
#' par(mfrow=c(3, 3))
#' for(edge in 0:5){
#'     plot_resid(stem=paste0("treemix.3.", edge),
#'     pop_order="pop_order.txt")
#'     }
#'
#' @export
