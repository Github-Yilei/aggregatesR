#' read_treemix
#' @description Read TreeMix output files.
#'
...
#'
#' @examples for m in {0..5};
#' @examples do
#' @examples     for i in {1..10};
#' @examples     do
#' @examples     s=$RANDOM
#' @examples     echo "treemix -i treemix_in.gz -o result1/treemix.${i}.${m} -root outgroup -m ${m} -global -noss -k 500 -seed ${s}"
#' @examples     done
#' @examples done > cmd.list
#' @examples df <- read_treemix(stem = "treemix.4.2")
#' @examples plot_treemix(df) + theme_bw()
#' @export
#'
...
#' plot_treemix
#' @description Plotted(ggplot2) is the structure of the graph inferred by TreeMix for sub-populations, allowing
